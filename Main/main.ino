...
#define TRIGGER_100MS   FREQUENCY/100
...
/*Objeto do tipo FlightControl para realizar o controle de velocidade de Roll*/
FlightControl yawVelPid(0.2f ,0.001f, 0.00f, 'y');
...
/*Variaveis para coletar o valor de velocidade angular de cada eixo*/ 
float pitchVel = 0;
float rollVel = 0;
float yawVel = 0;
...
void setup() {
  ...
  quadcopter.initializeMotors(PINMOTOR1,PINMOTOR2,PINMOTOR3,PINMOTOR4);
  delay(5000);
...
void loop() { 
  ...
  if(wifi_trigger){
    ...
    switch(estado_sistema){
      ...
      case 0:
        ...
        rollVelPid.setKp(ganhosPitch.kp);
        rollVelPid.setKd(ganhosPitch.kd);
        rollVelPid.setKi(ganhosPitch.ki);
  ...
  if(imu_trigger){ 
    ...
    /*Angulos de Pitch e Roll apos filtro complementar*/
    pitchVel = imu.getPitchVel();
    rollVel = imu.getRollVel();
    yawVel = imu.getYawVel();


    /*Executa a rotina de controle que atualiza o sinal de cada eixo baseado nos parametros lidos da IMU*/
    quadcopter.Control(rollVelPid, pitchVelPid, yawVelPid, rollVel, pitchVel, yawVel);
